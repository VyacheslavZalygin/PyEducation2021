...
for _ in range(n):
    ...
    is_continue = False
    # перебираю, начиная с последовательности максимальной длины
    for i in range(2):
        for j in range(2):
            ...
            if not is_continue and tail[i] == R[j]:
                # запоминаю, что продолжение должно быть другим числом в фишке
                tail[i] = R[1-j]
                # увеличиваю длину
                count[i] += 1
                ...
                if tail[1 - i] == R[1 - j]:
                    ...
                    count[1 - i] = 1
                # запоминаю конец второй последовательности
                tail[1 - i] = R[j]
                # меняю флаг
                is_continue = True
